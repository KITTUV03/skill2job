...
import { Radar, Mail, Lock, User, ArrowRight, Eye, EyeOff, Loader2, X } from 'lucide-react';
import { useApp } from '@/lib/store';

export default function SignupPage() {
  const router = useRouter();
  const { isAuthenticated, signupWithCredentials, loginWithGoogle, loginWithLinkedIn } = useApp();
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center p-4 bg-[#F8FAFC] dark:bg-[#0F172A] relative">
      <div className="w-full max-w-md p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-black text-slate-900 dark:text-white">Create Candidate Account</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Start matching your technical skills across 7+ verified job portals
          </p>
        </div>
        ...
        <div className="text-center pt-2 text-xs text-slate-600 dark:text-slate-400">
          ...
          <Link href="/login" className="font-extrabold text-primary-600 dark:text-primary-400 hover:underline">
            Sign In Instead
          </Link>
        </div>
          ...
          <div className="w-full max-w-sm p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-black text-slate-900 dark:text-white">Sign up with Google</h3>
              </div>
              <button
