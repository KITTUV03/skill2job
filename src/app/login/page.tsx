...
import { Radar, Mail, Lock, ArrowRight, Eye, EyeOff, ShieldCheck, KeyRound } from 'lucide-react';
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center p-4 bg-gradient-hero relative">
      ...
      <div className="w-full max-w-md p-8 rounded-3xl glass-card bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-extrabold text-slate-900 dark:text-white">Sign In to Your Account</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">Access personalized AI recommendations & multi-portal harvesters</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all flex items-center justify-center gap-2 shadow-sm"
          >
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="bg-white dark:bg-slate-900 px-3 text-[11px] text-slate-400 uppercase font-semibold absolute">Or credentials</span>
        </div>
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between pt-1">
            <div className="flex items-center gap-2">
              <input type="checkbox" id="remember" defaultChecked className="rounded accent-blue-600" />
          ...
          >
            <span>Log In to RoleRadar</span>
            <ArrowRight className="w-4 h-4" />
          </button>
        </form>

        <p className="text-center text-xs text-slate-500">
          Don't have an account? <Link href="/signup" className="text-blue-600 dark:text-blue-400 font-bold hover:underline">Create candidate account</Link>
        </p>
      ...
      {showForgotModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
          <div className="w-full max-w-sm p-6 rounded-3xl glass-card bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4">
            <div className="flex items-center gap-2 text-sm font-bold text-slate-900 dark:text-white">
              <KeyRound className="w-5 h-5 text-purple-500" />
              <span>Reset or Set New Password</span>
            </div>
            <p className="text-xs text-slate-500">Enter your email and specify a new security password for your account.</p>
            ...
            <form onSubmit={handlePasswordResetSubmit} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700"
                />
              </div>
